...
impl ViewSettingsPanel {
    /// Render the view-settings panel (layer controls). Collapsed into this
    /// citizen file from the former `ui/layer_controls.rs`.
    pub fn show(&self, ui: &mut egui::Ui, app: &mut crate::CopperForgeApp) {
        show_layers_panel(ui, app, &app.services.logger_state.clone(), &app.services.log_colors.clone());
    }
}

fn show_layers_panel<'a>(    ui: &mut egui::Ui,
