...
impl SettingsPanel {
    /// Render the application settings panel: units, timezone / clock format,
    /// preferred project directory, and language. The rendering lives here in
    /// the citizen file (collapsed from the former `ui/settings_panel.rs`).
    pub fn show(&self, ui: &mut egui::Ui, app: &mut crate::CopperForgeApp) {
        // Own clones (Arc-backed) bound to locals so the logger's borrows live
        // for the whole method while `app` is still used mutably below.
